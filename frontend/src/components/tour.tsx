...
export function TourProvider({
  ...
  const updateElementPosition = useCallback((shouldScroll = false) => {
    if (currentStep >= 0 && currentStep < steps.length) {
      ...
      const attemptUpdate = (retryCount = 0) => {
        ...
        if (element) {
          const rect = element.getBoundingClientRect();
          const scrollFlag = shouldScroll === true;

          if (scrollFlag && lastScrolledStepRef.current !== currentStep) {
            // Check visibility relative to both window viewport and any scrollable container parents
            const isVisible = (() => {
              if (
                rect.top < 0 ||
          ...
          // Retry every 50ms up to 10 times to wait for React route transitions
  ...
  useEffect(() => {
    ...
    window.addEventListener("resize", handleUpdateOnly, true);
    window.addEventListener("scroll", handleUpdateOnly, true);

    return () => {
      window.removeEventListener("resize", handleUpdateOnly, true);
      window.removeEventListener("scroll", handleUpdateOnly, true);
    };
...
export function TourAlertDialog({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (isOpen: boolean) => void }) {
  const { startTour, steps, isTourCompleted, currentStep, hasTours, setIsTourCompleted } = useTour();

  if (isTourCompleted || (steps.length === 0 && !hasTours) || currentStep > -1) {
    return null;
  }
  const handleSkip = async () => {
    setIsTourCompleted(true);
    localStorage.setItem("bluestep-tour-completed", "true");
    setIsOpen(false);
  };
    ...
    <AlertDialog open={isOpen}>
      <AlertDialogContent className="max-w-md p-6">
        ...
        <div className="mt-6 space-y-3">
          <Button onClick={() => { startTour(); setIsOpen(false); }} className="w-full">
            Start Tour
          </Button>
